...
if [ $# -ne 3 ]; then echo "Usage: $0 <scriptLocation> <inputFolder (/data/data03/wang/input/20150119_mcsa/preprocessing/fmri/)> <outputFolder(.../preprocessing/fmri_s/)>"; exit; fi

mkdir -p $3
for input_file in `ls $2/*.mnc*`; do
	output_file=`basename ${input_file}`
	if [ -f "$3/${output_file}" ]; then continue; fi
